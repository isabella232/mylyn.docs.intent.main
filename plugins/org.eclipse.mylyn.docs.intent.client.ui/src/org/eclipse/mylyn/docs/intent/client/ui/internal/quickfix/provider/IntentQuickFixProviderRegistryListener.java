 ...
 */
public class IntentQuickFixProviderRegistryListener implements IRegistryChangeListener {

	/** Name of the extension point to parse for extensions. */
	public static final String INTENTFIX_EXTENSION_POINT = "org.eclipse.mylyn.docs.intent.client.ui.quickfixprovider"; //$NON-NLS-1$

	/** Name of the extension point's "Save Extension Description" tag. */
	private static final String INTENTFIX_EXTENSION_TAG = "IntentQuickFixProvider"; //$NON-NLS-1$
	...
	public void init() {
		...
		registry.addRegistryChangeListener(this, INTENTFIX_EXTENSION_POINT);
	...
	private void parseExtension(IExtension extension) {
		...
		for (IConfigurationElement elem : configElements) {
			if (INTENTFIX_EXTENSION_TAG.equals(elem.getName())) {
				try {
					IntentQuickFixProviderRegistry.addExtension(new IntentQuickFixProviderDescriptor(elem));
	...
	public void parseInitialContributions() {
		...
		for (IExtension extension : registry.getExtensionPoint(INTENTFIX_EXTENSION_POINT).getExtensions()) {
			parseExtension(extension);
		}
	}
			...
			for (IConfigurationElement elem : configElements) {
				if (INTENTFIX_EXTENSION_TAG.equals(elem.getName())) {
					final String extensionClassName = elem
							.getAttribute(IntentQuickFixProviderDescriptor.INTENTFIX_EXTENSION_CONTRIBUTED_CLASS_NAME);
					IntentQuickFixProviderRegistry.removeExtension(extensionClassName);
				}
			}
		}
	}
	...
	public void dispose() {
		...
		IntentQuickFixProviderRegistry.clearRegistry();
	}
}
