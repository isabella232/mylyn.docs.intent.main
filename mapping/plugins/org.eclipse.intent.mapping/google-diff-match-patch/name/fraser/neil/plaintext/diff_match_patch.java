 ...
 * <p>
 * <b>Note :</b> This is a copy of the diff_match_patch class
 * https://code.google.com/p/google-diff-match-patch
 * /source/browse/trunk/java/name/fraser/neil/plaintext/diff_match_patch.java?spec=svn89&r=106
 * </p>
 * Related CQ: https://dev.eclipse.org/ipzilla/show_bug.cgi?id=7404
 */
// CHECKSTYLE:OFF
public class diff_match_patch {
	...
	private String[] diff_halfMatchI(String longtext, String shorttext, int i) {
		...
		while ((j = shorttext.indexOf(seed, j + 1)) != -1) {
			...
			if (best_common.length() < suffixLength + prefixLength) {
				best_common = shorttext.substring(j - suffixLength, j) + shorttext.substring(j, j
						+ prefixLength);
		...
		while (thisDiff != null) {
			if (thisDiff.operation == Operation.EQUAL) {
				...
				if (lastequality != null && (lastequality.length() <= Math.max(length_insertions1,
						length_deletions1)) && (lastequality.length() <= Math.max(length_insertions2,
								length_deletions2))) {
		...
		while (thisDiff != null) {
			if (prevDiff.operation == Operation.DELETE && thisDiff.operation == Operation.INSERT) {
				...
				if (overlap_length1 >= overlap_length2) {
					if (overlap_length1 >= deletion.length() / 2.0 || overlap_length1 >= insertion.length()
							/ 2.0) {
					...
					if (overlap_length2 >= deletion.length() / 2.0 || overlap_length2 >= insertion.length()
							/ 2.0) {
		...
		while (thisDiff != null) {
			if (thisDiff.operation == Operation.EQUAL) {
				...
				if (lastequality != null && ((pre_ins && pre_del && post_ins && post_del) || ((lastequality
						.length() < Diff_EditCost / 2) && ((pre_ins ? 1 : 0) + (pre_del ? 1 : 0) + (post_ins
								? 1
								: 0) + (post_del ? 1 : 0)) == 3))) {
					// System.out.println("Splitting: '" + lastequality + "'");
					// Walk back to offending equality.
		...
		while (nextDiff != null) {
			if (prevDiff.operation == Operation.EQUAL && nextDiff.operation == Operation.EQUAL) {
				...
				if (thisDiff.text.endsWith(prevDiff.text)) {
					...
					thisDiff.text = prevDiff.text + thisDiff.text.substring(0, thisDiff.text.length()
	...
	/**
	 * loc is a location in text1, compute and return the equivalent location in text2. e.g. "The cat" vs "The
	 * big cat", 1->1, 5->8
	 * 
	 * @param diffs
	 *            LinkedList of Diff objects.
	...
	protected int match_bitap(String text, String pattern, int loc) {
		assert (Match_MaxBits == 0 || pattern
				.length() <= Match_MaxBits) : "Pattern too long for this application.";
	...
	protected void patch_addContext(Patch patch, String text) {
		...
		while (text.indexOf(pattern) != text.lastIndexOf(pattern) && pattern.length() < Match_MaxBits
		...
		for (Diff aDiff : diffs) {
			...
			switch (aDiff.operation) {
				...
				case DELETE:
					...
					break;
				case EQUAL:
					if (aDiff.text.length() <= 2 * Patch_Margin && !patch.diffs.isEmpty() && aDiff != diffs
							.getLast()) {
		...
		for (Patch aPatch : patches) {
			...
			if (start_loc == -1) {
				...
				if (text1.equals(text2)) {
					...
					text = text.substring(0, start_loc) + diff_text2(aPatch.diffs) + text.substring(start_loc
							+ text1.length());
				} else {
				...
				while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin) {
					...
					if (diff_type == Operation.INSERT) {
						...
						if (diff_text.equals(bigpatch.diffs.getFirst().text)) {
							...
							bigpatch.diffs.getFirst().text = bigpatch.diffs.getFirst().text.substring(
									diff_text.length());
						}
					}
				}
